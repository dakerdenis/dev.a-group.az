@php
    /** @var \App\Models\StaticPage $staticPage */
    \Illuminate\Support\Facades\View::share('subTitle', $staticPage->subtitle );
    \Illuminate\Support\Facades\View::share('page_title', $staticPage->title );
@endphp
    ...
    <section class="text-content">
        <div class="main-container">
            <div class="wysiwyg animate-on-scroll animate__animated" data-animation="fadeIn">
                {!! html_entity_decode($staticPage->description) !!}
            </div>

            @if($staticPage->form)
                {!! $staticPage->getForm()->renderForm() !!}
            @endif

            @if($staticPage->files->count())
                <div class="download-links">
                    <h2>{{ __('site.static_pages_files') }}</h2>
            ...
            @endif
            @if($staticPage->form === \App\Forms\ComplaintForm::class)
                {!! (new \App\Forms\CheckComplaintForm($staticPage))->renderForm() !!}
            @endif
        </div>
    </section>

    @if($staticPage->usefulLinks->count())
        <section class="gray-section recommended-links">
            <div class="main-container">
                <h2>{{ __('site.static_pages_useful_links') }}</h2>

                <div class="links">
                    @foreach($staticPage->usefulLinks as $link)
                        <a href="{{ \Illuminate\Support\Str::startsWith('http', $link->link) ? $link->link : \Mcamara\LaravelLocalization\Facades\LaravelLocalization::getLocalizedURL(App::getLocale(), $link->link) }}" class="animate-on-scroll animate__animated" data-animation="zoomIn">
            ...
            <div class="main-container">
                ...
                <div class="swiper-container gallerySwiper">
                    <div class="swiper-wrapper">
                        @foreach($staticPage->gallery->getMedia()->chunk(6) as $chunk)
