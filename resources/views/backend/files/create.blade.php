...
@section('content')
    ...
    @include('backend.partials.title_breadcrumbs', ['title' => isset($file) ? 'Update ' . $file->title : 'Create File', 'breadcrumbs' => [['active' => false, 'title' => 'Files', 'link' => route('backend.file.index')],], 'buttons' => null])
        <form
            class="content-form"
            action="{{isset($file) ? route('backend.file.update', [$file]) : route('backend.file.store')}}"
                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                        ...
                                        <div class="tab-content" id="myTabContent">
                                            @foreach(LaravelLocalization::getLocalesOrder() as $localeCode => $properties)
                                                <div class="tab-pane fade {{$loop->first ? 'active show' : ''}}"
                                                    ...
                                                    <div class="mb-0 m-t-30">
                                                        <div class="form-group">
                                                            ...
                                                            <input
                                                                ...
                                                                placeholder="Title"
                                                                value="{{old('title:'.$localeCode, (isset($file) && $file->translate($localeCode)) ? $file->translate($localeCode)->title : '')}}"
                                                                id="title">
                                                    ...
                                                    <div class="row mt-3">
                                                        <div class="col-md-6">
                                                            <label for="formFile:{{$localeCode}}" class="form-label">File</label>
                                                            <input name="file:{{$localeCode}}" class="form-control" type="file" id="formFile:{{$localeCode}}">
                                                            @if(isset($file) && $file->translate($localeCode)->path)
                                                                <p class="mt-1 mb-0">Current</p>
                                                                <p>
                                                                    <a href="{{ Storage::disk('public')->url($file->translate($localeCode)->path) }}">{{ $file->translate($localeCode)->path }}</a>
                                                                </p>
                                                            @endif
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="card-footer">
                                                ...
                                                <div class="media">
                                                    ...
                                                    <div class="media-body text-center switch-outline">
                                                        <label class="switch">
                                                            ...
                                                            <input type="checkbox" value="1" {{ old('active', isset($file) && !$file->active) ? '' : 'checked' }} data-original-title=""
                                        ...
                                        <button
                                            class="btn btn-primary">{{isset($file) ? 'Update' : 'Create'}}</button>
                                        <a style="color: white" href="{{route('backend.file.index')}}"
