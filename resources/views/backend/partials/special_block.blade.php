                ...
                <div class="col-md-6">
                    <div class="mb-0 m-t-30">
                        <div class="form-group">
                            <label for="special[{{isset($special) ? (is_array($special) ? $id : $special->id) : $id}}][preview]" class="col-form-label">Image</label>
                            <input type="hidden" name="image[{{isset($special) ? (is_array($special) ? $id : $special->id) : $id}}]" value="{{old('image.'.$id, isset($special) && is_object($special) && $special->getFirstMediaUrl('preview') ? $special->getFirstMediaUrl('preview') : '')}}">
                            <input type="hidden" class="url" name="special[{{isset($special) ? (is_array($special) ? $id : $special->id) : $id}}][preview_url]" id="preview_url-{{$id}}">
                            <input type="file" name="special[{{isset($special) ? (is_array($special) ? $id : $special->id) : $id}}][preview]" id="input-file-now-{{$id}}" class="dropify" data-default-file="{{old('image.'.(isset($special) ? (is_array($special) ? $id : $special->id) : $id), isset($special) && is_object($special) && $special->getFirstMediaUrl('special') ? $special->getFirstMediaUrl('special') : '')}}" data-show-remove="false"/>
                        </div>
                    </div>
                </div>
