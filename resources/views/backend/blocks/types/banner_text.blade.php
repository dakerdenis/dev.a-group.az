...
@section('content')
    ...
    @include('backend.partials.title_breadcrumbs', ['title' => isset($block) ? 'Update ' . $block->title : 'Create Banner Block', 'breadcrumbs' => [['active' => false, 'title' => 'Blocks', 'link' => route('backend.blocks.index')],], 'buttons' => null])
    <form
                                            ...
                                            <div class="tab-pane fade {{$loop->first ? 'active show' : ''}}"
                                                ...
                                                <div class="mb-0 m-t-30">
                                                    ...
                                                    <div class="form-group mt-3">
                                                        <label for="data:{{$localeCode}}[description]" class="col-form-label">Text</label>
                                                        <textarea rows="5" id="data:{{$localeCode}}[description]" class="form-control" name="data:{{$localeCode}}[description]">{{old('data:'.$localeCode.'.description', (isset($block) && $block->translate($localeCode)) ? $block->translate($localeCode)->data['description'] : '')}}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
...
{{--                                            <div class="col-md-6">--}}
{{--                                                <div class="form-group">--}}
{{--                                                    <label for="slug" class="col-form-label">Banner</label>--}}
{{--                                                    <input type="hidden" name="delete_images[]" id="deleteImages">--}}
{{--                                                    <input type="hidden" name="preview_url" class="url" id="preview_url">--}}
{{--                                                    <input type="file" data-file-id="{{isset($block) && $block->getFirstMedia('preview') ? $block->getFirstMedia('preview')->id : '' }}" name="preview" id="input-file-now" class="dropify" data-default-file="{{isset($block) && $block->getFirstMediaUrl('preview') ? $block->getFirstMediaUrl('preview') : '' }}"/>--}}
{{--                                                </div>--}}
{{--                                            </div>--}}
{{--                                        </div>--}}
                                    <button
